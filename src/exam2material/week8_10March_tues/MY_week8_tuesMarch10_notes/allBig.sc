...
@pure def allBig[T](P: T=>B @pure, Q: T=>B @pure, R: T=>B @pure, B: T=>B @pure, C: T=>B @pure): Unit = {
  Deduce(
    ...
    Proof(
      ...
      //no individual available, cna't use AllE on premises yet

      //use AllI subproof to create my conclusion
      4 Let ((random: T) => SubProof(  
        //use ImplyI pattern to build my goal (the implies)
        5 SubProof(
          6 Assume(P(random) | Q(random)),

          //use NegI pattern to build my goal (a negation)
          7 SubProof(
            8 Assume(R(random)),
            ...
            //try OrE to see if we can get to goal of F in both side
            15 SubProof(
              16 Assume (P(random)),
          ...
          24 (!R(random)) by NegI(7) //can get this by getting a false when assuming R(random)
          //goal: !R(random)
        ),
